...
#define FIREBASE_AUTH "EnterFirebaseSecretKey"                      // the secret key generated from firebase Service accounts>Database secrets
                      
#define WIFI_SSID     "EnterSSIDName"                                // put your home or public wifi name 
#define WIFI_PASSWORD "EnterWiFiPassword"                            // put your password of wifi ssid
...
void NodeMCU_Firebase::Loop()
{
  ...
  switch (m_State)
  {
    ...
    case WRITING_STATE:  
      ...
      Firebase.setFloat("/meteoarchive/" + smc + "/humidity", m_currentHumidity);
      Firebase.setFloat("/meteoarchive/" + smc + "/temperature", m_currentTemperature);
      Firebase.setInt("/meteoarchive/" + smc + "/pressure", m_currentPressure);   
      Firebase.setInt("/meteoarchive/" + smc + "/dayofweek", m_dayOfWeek);      
      Firebase.setString("/meteoarchive/" + smc + "/time", m_time);      
      m_NextNodeMCU_FirebaseState = IDLE_STATE;
      mc++;
      delay(1000);
      break;
